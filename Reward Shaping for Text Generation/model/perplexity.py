...
class Perplexity(evaluate.Metric):
    """
    huggingface Perplexity metric modified in order to avoid redundant model
    load and destruction upon each evaluation call.
    """
    def __init__(self, *args, model_id='EleutherAI/gpt-j-6B', **kwargs):
    ...
    def _compute(
        self, predictions, batch_size: int = 16, add_start_token: bool = True, max_length: int = None, verbose: bool = False,
    ):
        ...
        for start_index in logging.tqdm(range(0, len(encoded_texts), batch_size), disable=not verbose):
