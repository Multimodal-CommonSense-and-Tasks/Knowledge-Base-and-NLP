...
def write_ranking(corpus_indices, corpus_scores, q_lookup, ranking_save_file, for_pyserini):
    with open(ranking_save_file, 'w') as f:
        for qid, q_doc_scores, q_doc_indices in zip(q_lookup, corpus_scores, corpus_indices):
            score_list = [(s, idx) for s, idx in zip(q_doc_scores, q_doc_indices)]
            score_list = sorted(score_list, key=lambda x: x[0], reverse=True)
            for index, (s, idx) in enumerate(score_list):
                if for_pyserini:
                    f.write(f"{qid}\tQ0\t{idx}\t{index+1}\t{s}\tdense\n")
                else:
                    f.write(f'{qid}\t{idx}\t{s}\n')                    
...
def main():
    ...
    parser.add_argument('--for_pyserini', action='store_true')
    ...
    if args.save_text:
        write_ranking(psg_indices, all_scores, q_lookup, args.save_ranking_to, args.for_pyserini)
    else:
