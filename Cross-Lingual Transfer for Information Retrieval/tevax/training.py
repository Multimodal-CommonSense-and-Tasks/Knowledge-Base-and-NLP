...
def retriever_train_step_naive(state, queries, passages, dropout_rng, axis='device'):
...
def retriever_train_step_contrastive(
    state,
    queries,
    passages,
    codemixed_queries,
    ...
    axis="device",
    cm_target=None,
    cm_loss_weight_q=0.0,
    cm_loss_weight_p=0.0,
):
    assert (
        cm_target == None or cm_target == "q" or cm_target == "p" or cm_target == "pq"
    ), "Cannot set both q_only and p_only to True"
    q_dropout_rng, p_dropout_rng, new_dropout_rng = jax.random.split(dropout_rng, 3)
    ...
    def compute_loss(params):
        ...
        p_reps = state.apply_fn(**passages, params=params.p_params, dropout_rng=p_dropout_rng, train=True)[0][:, 0, :]
        if cm_target == "q":
            cm_q_reps = state.apply_fn(
                **codemixed_queries,
                params=params.q_params,
                dropout_rng=q_dropout_rng,
                train=True
            )[0][:, 0, :]
            return (
                jnp.mean(p_contrastive_loss(q_reps, p_reps, axis=axis))
                + jnp.mean(p_contrastive_loss(q_reps, cm_q_reps, axis=axis))
                * cm_loss_weight_q
            )
        elif cm_target == "p":
            cm_p_reps = state.apply_fn(
                **codemixed_passages,
                params=params.p_params,
                dropout_rng=p_dropout_rng,
                train=True
            )[0][:, 0, :]
            return (
                jnp.mean(p_contrastive_loss(q_reps, p_reps, axis=axis))
                + jnp.mean(p_contrastive_loss(p_reps, cm_p_reps, axis=axis))
                * cm_loss_weight_p
            )
        elif cm_target == "pq":
            cm_q_reps = state.apply_fn(
