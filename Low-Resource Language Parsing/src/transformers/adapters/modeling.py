...
class MLPFusion(nn.Module):
    ...
    def __init__(
        ...
        """
        mode: "query" or "key" or "query+key"
        """
        ...
        total_input_size = 0
        if "query" in mode: # adapter_input
            total_input_size += dense_size
        if "key" in mode: # adapter_outputs
            total_input_size += dense_size * num_adapters

        if mlp_depth == 1:
            self.mlp = nn.Linear(total_input_size, num_adapters, bias=False)
            self.mlp.apply(Adapter.init_bert_weights)
        elif mlp_depth == 2:
            print('Using 2 layer MLP')
            mid_size = int(math.sqrt(num_adapters * total_input_size))
            self.mlp = nn.Sequential(
                nn.Linear(total_input_size, mid_size),
                nn.GELU(),
                nn.Linear(mid_size, num_adapters)
            )
            self.mlp.apply(Adapter.init_bert_weights)
        else:
    ...
    def forward(self, query, key, value, residual=None, output_attentions: bool = False):
        """
        query: adapder inputs
        key, value: adapter outputs
        """
        if residual is not None and self.config["residual_before"]:
            value += residual[:, :, None, :].repeat(1, 1, value.size(2), 1)

        inputs = []
        if 'query' in self.mode:
            inputs.append(query) # (batch_size, seq_len, hidden_size)
        if 'key' in self.mode:
            inputs.append(key.reshape(*key.shape[:2], -1)) # (batch_size, seq_len, num_adapters*dim*num_heads)
        len_set = set([x.shape[1] for x in inputs])
        len_set.add(value.shape[1])
        if len(len_set) > 1: # ex. prefix_tuning with different seq_len
            inputs = [torch.mean(i, dim=1) for i in inputs]
        inputs = torch.cat(inputs, dim=-1)
        adapter_weights = torch.softmax(self.mlp(inputs), dim=-1) # (batch_size, toks, number-of-adapters)
        ...
        if adapter_weights.dim() == 2:
            adapter_weights = adapter_weights.unsqueeze(1).expand(-1, value_layer.shape[1], -1)
        context_layer = torch.squeeze(torch.matmul(adapter_weights.unsqueeze(2), value_layer), dim=2)
        ...
        if residual is not None and not self.config["residual_before"]:
...
class AvgFusion(nn.Module):
    ...
    def forward(self, query, key, value, residual=None, output_attentions: bool = False):
        ...
        context_layer = torch.mean(value, dim=2)
...
class BertFusion(nn.Module):
    ...
    def __init__(
        ...
        #         "heads (%d)" % (config.hidden_size, config.num_attention_heads))
        self.config = config
        self.invert_attention_score = invert_attention_score
    ...
    def forward(self, query, key, value, residual=None, output_attentions: bool = False):
        ...
        use_mean = query_layer.size(1) != key_layer.size(1)
        if use_mean:
            query_layer = torch.mean(query_layer, dim=1, keepdim=True)
            key_layer = torch.mean(key_layer, dim=1, keepdim=True)

        attention_scores = torch.squeeze(torch.matmul(query_layer.unsqueeze(2), key_layer.transpose(-2, -1)), dim=2)
        if use_mean:
            attn_score_shape = list(attention_scores.size())
            attn_score_shape[1] = int(value_layer.size(1))
            attention_scores = attention_scores.broadcast_to(attn_score_shape)

        attention_scores = self.dropout(attention_scores)
        if self.invert_attention_score:
