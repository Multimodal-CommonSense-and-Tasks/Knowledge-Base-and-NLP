...
class InvertibleAdaptersMixin:
    ...
    def add_invertible_adapter_fusion(self, adapter_names: Union[List, str]):
    ...
    def delete_invertible_adapter_fusion(self, adapter_names: Union[List, str]):
    ...
    def invertible_adapters_forward(self, hidden_states, rev=False):
        # TODO: Currently no fusion over invertible adapters, takes only very first language adapter position
        if self.config.adapters.active_setup is not None and len(self.config.adapters.active_setup) > 0:
            if hasattr(self.config, 'inv_fusion') and self.config.inv_fusion:
                adapter_setup = self.config.adapters.active_setup
                if isinstance(adapter_setup, Stack):
                    adapter_setup = adapter_setup[0]
                hidden_states = self.invertible_adapter_fusion(adapter_setup, hidden_states, rev=rev)
            else:
                first_adapter = self.config.adapters.active_setup.first()
                if first_adapter in self.invertible_adapters:
    ...
    def invertible_adapter_fusion(self, adapter_setup: Fuse, hidden_states, rev):
        """
        ...
        for adapter_block in adapter_setup:
            ...
            elif adapter_block in self.invertible_adapters:
                up_list.append(self.invertible_adapters[adapter_block](hidden_states, rev=rev))
...
class ModelAdaptersMixin(PushAdapterToHubMixin, ABC):
    ...
    def add_adapter_fusion(
        ...
        unwrapped_model = self if not hasattr(self, self.base_model_prefix) else getattr(self, self.base_model_prefix)
        if isinstance(unwrapped_model, InvertibleAdaptersMixin) and hasattr(unwrapped_model.config, 'inv_fusion') and unwrapped_model.config.inv_fusion:
            unwrapped_model.add_invertible_adapter_fusion(adapter_names)
        if set_active:
