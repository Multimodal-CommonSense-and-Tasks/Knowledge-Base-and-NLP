...
if is_torch_tpu_available(check_device=False): # Critical : prevents the device
    import torch_xla.core.xla_model as xm


def mkdir(*args, **kwargs):
    if is_torch_tpu_available():
        mk = xm.is_master_ordinal()
    else:
        mk = True
    if mk:
        os_mkdir(*args, **kwargs)
...
class WeightsLoaderHelper:
    ...
    def save_weights_config(self, save_directory, config, meta_dict=None):
        ...
        if is_torch_tpu_available():
            save_config = xm.is_master_ordinal()
        else:
            save_config = True
        if save_config:
    ...
    def save_weights(self, save_directory, filter_func):
        ...
        if is_torch_tpu_available():
            ...
            save_ftn = torch.save
        save_ftn(state_dict, output_file)
