...
class UDTrainingArguments(TrainingArguments):
    ...
    **which relate to the training loop itself**.
    ...
    train_both_model_adapter: str = field(default=None, metadata={"help": "how to train model with adapter",
                                                                  "choices": ["model+la+ta", "model+ta", "model+la"]})
    model_learning_rate_correct: float = field(default=3e-5)
    fusion_learning_rate: float = field(default=None)
    head_learning_rate: float = field(default=None)
    lang_learning_rate: float = field(default=None)
    train_calc_weight_step: Optional[int] = field(default=0)
    eval_calc_weight_step: Optional[int] = field(default=0)
    train_adapter_weight: List[float] = field(default=None)
    eval_adapter_weight: List[float] = field(default=None)
    both_arc_rel_entropy: bool = field(default=False)
    entropy_lr: float = field(default=10)
    weight_norm_method: str = field(default='softmax')
    layerwise_entropy: bool = field(default=False)
    disable_invertible_fusion: bool = field(default=False)

    def __post_init__(self):
        super().__post_init__()
        if self.fusion_learning_rate is None:
            self.fusion_learning_rate = self.learning_rate
        if self.lang_learning_rate is None:
...
class DependencyParsingTrainer(Trainer):
    ...
    def evaluate(
        ...
        result = {'eval_' + k: v for k, v in output.metrics.items()}
