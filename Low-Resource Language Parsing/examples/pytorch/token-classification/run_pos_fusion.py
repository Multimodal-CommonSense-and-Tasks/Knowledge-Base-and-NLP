...
from transformers.adapters.configuration import AdapterConfig, IA3Config, ConfigUnion, PrefixTuningConfig, PfeifferConfig, LoRAConfig, PfeifferInvConfig, DynamicAdapterFusionConfig, StaticAdapterFusionConfig
...
class DataTrainingArguments:
    ...
    fusion_learning_rate: float = field(default=None)
...
def get_config_tok_model(model_args, data_args, num_labels, label_to_id, label_map, adapter_args):
...
def get_pos_model_and_dataset(model_args, data_args, training_args, adapter_args):
    labels = UPOS_LABELS
    label_map: Dict[int, str] = {i: label for i, label in enumerate(labels)}
    label_to_id = {label: i for i, label in enumerate(labels)}
    num_labels = len(labels)

    config, tokenizer, model = get_config_tok_model(model_args, data_args, num_labels, label_to_id, label_map, adapter_args)
    ...
    return config, model, tokenizer, train_dataset, eval_dataset, predict_dataset, label_map, compute_metrics, data_collator, num_labels


def setup_adapters(model, tokenizer, model_args, data_args, adapter_args, num_labels, training_args):
    # Setup adapters
    adapter_config = create_adapter_config(adapter_args)
    lang_adapter_config = create_adapter_config(adapter_args, use_lang=True)
    ...
    def normalize_name(s, limit_len=143 // 4):
        s = s.replace("@", "")
        s = s.replace("/", "")
        s = s.replace(".", "")
        # if len(s) > limit_len:
    ...
    if 'lang_fuse' in adapter_args.fuse_mode:
        ...
        if adapter_args.load_adapter:
            ...
            task_adapter_name = data_args.task_name
        ...
        if adapter_args.fuse_mode == 'lang_fuse_train_fuse_and_task':
            ...
            groups.extend(
                get_optim_grouped_params_only_requires_grad(model, data_args.fusion_learning_rate, training_args.weight_decay, consider_adapter_params=True,
...
def main():
    ...
    if data_args.fusion_learning_rate is None:
        data_args.fusion_learning_rate = training_args.learning_rate
    ...
    if not data_args.pos_data_dir and not data_args.test_file.endswith('conllu'):
        ...
        config, model, tokenizer, train_dataset, eval_dataset, predict_dataset, label_map, compute_metrics, data_collator, num_labels = get_pos_model_and_dataset(
            model_args, data_args, training_args, adapter_args)

    ############## setup adapters

    # Setup adapters
    setup_adapters(model, tokenizer, model_args, data_args, adapter_args, num_labels, training_args)
