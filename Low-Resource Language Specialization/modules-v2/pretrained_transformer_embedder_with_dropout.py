...
@TokenEmbedder.register("pretrained_transformer_with_dropout")
class PretrainedTransformerEmbedderWithDropout(TokenEmbedder):
    """
    ...
    def __init__(
        ...
        *,
        ...
        if sub_module:
            ...
            self.transformer_model = getattr(
                self.transformer_model, sub_module
            )
        ...
        self._scalar_mix: Optional[ScalarMixWithDropout] = None
    ...
    def forward(
        ...
        if fold_long_sequences:
            ...
            (
                ...
                segment_concat_mask,
                type_ids,
            ) = self._fold_long_sequences(
                token_ids, segment_concat_mask, type_ids
            )

        transformer_mask = (
            segment_concat_mask if self._max_length is not None else mask
        )
    ...
    def _unfold_long_sequences(
        ...
        if not (
            lengths_to_mask(seq_lengths, mask.size(1), device) == mask
        ).all():
            raise ValueError(
        ...
        embeddings = embeddings[
            :,
            :,
            self._num_added_start_tokens : embeddings.size(2)
        ...
        # Shape: (batch_size,)
        num_effective_segments = (
            seq_lengths + self._max_length - 1
        ) // self._max_length
