...
@Head.register("tagger_parser")
class BiaffineDependencyParserWithLinearTagger(Head):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tagger_dropout = Dropout(tagger_dropout)
        self.tagger_linear = torch.nn.Linear(
            tagger_encoder.get_output_dim(), vocab.get_vocab_size("pos")
        )
        self.tagger_weight = tagger_weight
        self.parser_weight = parser_weight
        self.tagger_accuracy = CategoricalAccuracy()
        self.tagger_accuracy_words_only = CategoricalAccuracy()
    ...
    def forward(
        ...
        tagger_encoded_text = self.tagger_encoder(embedded_text_input, mask)
        tagger_batch = self.tagger_dropout(tagger_encoded_text)
        tagger_logits = self.tagger_linear(tagger_batch)

        pos_loss = 0.0
        if pos_tags is not None:
            pos_loss = sequence_cross_entropy_with_logits(
                tagger_logits, pos_tags, mask
            )
            self.tagger_accuracy(tagger_logits, pos_tags, mask)
            word_mask = mask.clone()
            for label in self.non_word_tags:
                label_mask = pos_tags.eq(label)
                word_mask = word_mask & ~label_mask
            self.tagger_accuracy_words_only(tagger_logits, pos_tags, word_mask)
        ...
        (
            ...
            arc_nll,
            tag_nll,
            _normalized_arc_logits,
            _normalized_head_tag_logits,
            _arc_raw_loss,
            _tag_raw_loss,
            _valid_positions,
        ) = self._parse(embedded_text_input, mask, head_tags, head_indices)

        parse_loss = arc_nll + tag_nll

        loss = self.parser_weight * parse_loss + self.tagger_weight * pos_loss
    ...
    def _parse(
        ...
        if head_indices is not None and head_tags is not None:
            ...
            if torch.any(head_tags > max_head_tag_index):
                logger.info(
                    "Unseen gold tag!  Skipping loss computation "
                    "to avoid tensor errors"
                )
                arc_nll = torch.tensor(0.0)
                tag_nll = torch.tensor(0.0)
                normalized_arc_logits = None
                normalized_head_tag_logits = None
                arc_raw = None
                tag_raw = None
    ...
    def _mst_decode(
        ...
        # Shape (batch_size, sequence_length, sequence_length)
        normalized_arc_logits = F.log_softmax(attended_arcs, dim=2).transpose(
            1, 2
        )
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        result["tagger_accuracy"] = self.tagger_accuracy.get_metric(reset)
        result[
            "tagger_accuracy_words_only"
        ] = self.tagger_accuracy_words_only.get_metric(reset)
        return result
