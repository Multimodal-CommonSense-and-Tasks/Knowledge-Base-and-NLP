...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint, skip_optim=False):
  ...
  for x in init_vars:
    ...
    if name not in name_to_variable:
      tf.compat.v1.logging.info('skip {} -- not in var'.format(name))
      continue

    if skip_optim and any([optim_name in name for optim_name in ['Momentum', 'RmsProp', 'Adam', 'global_step']]):
      tf.compat.v1.logging.info('skip {} -- optim_var'.format(name))
      continue
