...
class CategoricalAccuracy(Metric):
    ...
    def __call__(
        ...
        if not self._tie_break:
            ...
            if os.environ.get("WRITE_FILE_PREFIX", None) and self._top_k == 1:
                f_prefix = os.environ.get("WRITE_FILE_PREFIX")
                with open(f"{f_prefix}_top_k{self._top_k}", 'a', encoding='utf-8') as f:
                    for _something in top_k.clone().cpu().squeeze():
                        f.write(f"{int(_something)}\n")
                with open(f"{f_prefix}_correct{self._top_k}", 'a', encoding='utf-8') as f:
                    for _something in correct.clone().cpu().squeeze():
                        f.write(f"{int(_something)}\n")
        else:
