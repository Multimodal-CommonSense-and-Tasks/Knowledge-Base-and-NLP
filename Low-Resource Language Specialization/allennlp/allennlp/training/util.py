    ...
    with torch.no_grad():
        ...
        for batch in generator_tqdm:
            ...
            if os.environ.get("WRITE_FILE_PREFIX", None):
                ...
                with open(f"{f_prefix}_wordpieces", 'a', encoding='utf-8') as f:
                    for _s in batch['tokens']['transformer']['token_ids'].clone().cpu().view(-1):
                        f.write(f"{model.vocab._index_to_token['tags'][int(_s)]}\n")
