...
save_dir = f"data/distilled/cicero_counterfactual/frag_{args.n_frag}/{args.frag_index}"
if args.n_frag:
    frag_len = int(len(train_data)/args.n_frag)
    train_data = train_data[frag_len*args.frag_index:frag_len*(args.frag_index+1)]


with open(f"prompts/rg_focused_prompt/counterfactual.txt", "r") as f:
    prompt = f.read()
...
for d in train_data:
    ...
    for cur_turn_idx in range(3,len(dialogue)):
        ...
        string_context = context[-1] ## counterfactual context
        cur_example = f"\n{string_context}\nTarget:\n{target}\nRationale:\n"
